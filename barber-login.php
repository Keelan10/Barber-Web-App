...
<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
		...
		if($emailErr == "" && $passwordErr == "" ){
			...
			$sQuery = "SELECT * FROM barber WHERE email = '$email'  ";
			...
			if(($userResults)){//if exists
				...
				if(password_verify($password,$hashed_password)){
					$_SESSION['barber_username'] = $userResults["first_name"] . $userResults["last_name"];
					$_SESSION['barber_userid']=$userResults["barberid"];
                    header("Location: barber-calendar.php");
					die();
				}
				else
				{
					$Msg = "Your credentials seem to be wrong" ."<br>". "Try again or make sure you are a registered barber!";
					// echo $Msg;
				}
			}else{
				$Msg = "Your credentials seem to be wrong" ."<br>". "Try again or make sure you are a registered barber!";
